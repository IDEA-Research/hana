...
class GaussianDiffusion:
    ...
    def p_mean_variance(
        self, model, x, t, clip_denoised=True, dynamic_threshold=None,
        denoised_fn=None, model_kwargs=None, aug_t=None,
    ):
        """
        ...
        :param clip_denoised: if True, clip the denoised signal into [-1, 1].
            (static threshold)
        :param dynamic_threshold: if specified, the threshold for the denoised.
            (dynamic threshold, priority first)
        :param denoised_fn: if not None, a function which applies to the
            x_start prediction before it is used to sample. Applies before
            clip_denoised.
        :param model_kwargs: if not None, a dict of extra keyword arguments to
            pass to the model. This can be used for conditioning.
        :param aug_t: a batch of augmented timestep indices. 
        ...
        if model.model.__class__.__name__ == "SuperResText2ImUNet":
            model_output = model(x, self._scale_timesteps(t),
                                 aug_t=self._scale_timesteps(aug_t), **model_kwargs)
        else:
    ...
    def p_sample_loop(
        ...
        if "low_res" in model_kwargs and aug_t is not None:
            model_kwargs["low_res"] = self.q_sample(model_kwargs["low_res"], aug_t)
    ...
    def _vb_terms_bpd(
        self, model, x_start, x_t, t, clip_denoised=True, model_kwargs=None, aug_t=None
    ):
        """
        ...
        out = self.p_mean_variance(
            model, x_t, t, clip_denoised=clip_denoised, model_kwargs=model_kwargs, aug_t=aug_t
        )
        kl = normal_kl(
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None, aug_t=None, p2_loss=False):
        """
        ...
        :param aug_t: a batch of augmented timestep indices. 
            (used for noise conditional augmentation in super-resolution task)
        :param p2_loss: boolean, whether to use perceptual aware loss.
            refer to: https://arxiv.org/pdf/2204.00227.pdf
        ...
        if p2_loss:
            loss_weights = _extract_into_tensor(1. - self.alphas_cumprod, t, x_start.shape) 
        # if noise_conditional_augment enabled, use aug_t to augment low res img
        if "low_res" in model_kwargs and aug_t is not None:
            model_kwargs["low_res"] = self.q_sample(model_kwargs["low_res"], aug_t)
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if p2_loss:     
                terms["mse"] = mean_flat((target - model_output) ** 2 *loss_weights)
            else:
