...
# @Last Modified time: 2022-09-14 17:06:33
import time
import os
import os.path as op
from .io_common import generate_lineidx
    ...
    def _ensure_lineidx_loaded(self):
        ...
        if self._lineidx is None:
            with open(self.lineidx, 'r') as fp:
                lines = fp.readlines()
                self._lineidx = [int(i.strip()) for i in lines]

            if self.split_label:
                with open(self.label_lineidx, 'r') as fp:
                    lines = fp.readlines()
                    self._label_lineidx = [int(i.strip()) for i in lines]    
