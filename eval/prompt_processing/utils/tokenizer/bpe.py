...
class Encoder:
    def __init__(self, encoder, bpe_merges, errors="replace", text_ctx:int=None):
    ...
    def __call__(self, text):
        ...
        assert self.text_ctx is not None, "self attr `text_ctx` is not set"
        token, mask = self.padded_tokens_and_mask(token, self.text_ctx)
        return token, mask


def get_encoder(text_ctx:int=None):
